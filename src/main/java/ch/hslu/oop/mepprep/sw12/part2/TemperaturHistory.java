...
public class TemperaturHistory {
    ...
    public Temperatur maxStream() {
        return temperatures.stream()
                .max(Comparator.comparing(Temperatur::getKelvin))
                .orElse(null);
    }
    ...
    public Temperatur minStream() {
        return temperatures.stream()
                .min(Comparator.comparing(Temperatur::getKelvin))
                .orElse(null);
    }
    ...
    public Temperatur averageStream() {
        if(getCount() != 0) {
            float average = (float) temperatures.stream()
        ...
        throw new NullPointerException("TemperaturHistory empty");
    }
